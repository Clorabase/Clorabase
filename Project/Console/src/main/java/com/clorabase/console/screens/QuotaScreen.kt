...
fun QuotaInfoCard(quota: Quota) {
    Card(
        ...
        Column(
            ...
            val resetTimeMillis = quota.resetSeconds * 1000L
            val diffMillis = resetTimeMillis - currentTimeMillis
            val diffMinutes = (diffMillis / (1000 * 60)).coerceAtLeast(0)
            val diffHours = diffMinutes / 60
            val remainingMinutes = diffMinutes % 60

            val resetsIn = when {
                diffHours > 0 -> "${diffHours}h ${remainingMinutes}m"
                else -> "${remainingMinutes}m"
            }
            val resetTime = SimpleDateFormat("HH:mm", Locale.getDefault()).format(Date(resetTimeMillis))

            QuotaDetailRow("Remaining requests:", quota.remaining.toString())
            QuotaDetailRow("Resets at:", resetTime)
            QuotaDetailRow("Resets in:", resetsIn)
        }
    }
}
