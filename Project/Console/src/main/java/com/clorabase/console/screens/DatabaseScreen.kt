...
fun DatabaseScreen(viewModel: DatabaseViewModel = viewModel()) {
            ...
            is DatabaseUiState.Success -> {
                ...
                if (showCollectionDialog)
                    FolderDialog(
                        ...
                        onCreate = {
                            viewModel.onNewCollectionClick(it)
                ...
                LazyColumn(
                    ...
                    items(state.files) {
                        FileListItem(it) { clickedItem ->
                            when (clickedItem) {
                                is ListItem.Document -> {
                                    launcher.launch(Intent(context, DocumentActivity::class.java).apply {
                                        putExtra("path", clickedItem.doc.path)
                                        putExtra("name", clickedItem.doc.name)
                                        putExtra("mode", "update")
                                    })
                                }
                                is ListItem.Folder -> {
                                    viewModel.navigateIntoCollection(clickedItem.name)
                                }
                                else -> {}
                            }
                        }
    ...
    Dialog(onDismissRequest = onDismissRequest) {
        Surface(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                OutlinedTextField(
                    value = docName,
                    onValueChange = { docName = it.filter { char -> char.isDigit() || char.isLetter() } },
                ...
                Row(
                    ...
                    Button(onClick = {
                        if (docName.isNotBlank()) {
                            onCreate("$docName.doc", currentPath)
                            onDismissRequest()
                        }
                    }) {
                        Text("Create")
