...
public class ClorabaseStorage {
    ...
    public static void download(@NonNull String project,@NonNull String filename, @NonNull File directory, @NonNull ClorabaseStorageCallback listener) {
        new Thread(() -> Utils.download(directory, filename, project + "/Storage/" + filename, new Utils.AsyncCallback() {
            @Override
    ...
    public static void upload(@NonNull String project, @NonNull File file, @NonNull ClorabaseStorageCallback listener) {
        AndroidNetworking.upload("https://clorabase.herokuapp.com/github/upload")
                ...
                .build()
                .setUploadProgressListener((l, l1) -> listener.onProgress((int) ((l*100)/l1)))
    ...
    public static void delete(@NonNull String project, @NonNull String filename, @NonNull ClorabaseStorageCallback listener) {
        new Thread(() -> {
            try {
                Utils.repo.getFileContent(project + "/Storage/" + filename).delete("Deleted from the clorabase");
                listener.onComplete();
            } catch (IOException e) {
                listener.onFailed(e);
            }
