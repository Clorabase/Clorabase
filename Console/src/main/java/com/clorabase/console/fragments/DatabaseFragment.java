            ...
            try {
                ...
                getActivity().runOnUiThread(() -> {
                    ...
                    if (e.getCause() instanceof UnknownHostException)
                        makeText(getContext(), "Check your internet connection !", Toast.LENGTH_SHORT).show();
                    else if (e.getCause() instanceof FileNotFoundException)
                        makeText(getContext(), "Start by creating a document", Toast.LENGTH_SHORT).show();
                    else
                        makeText(getContext(), "An error occurred. Please report on the github", Toast.LENGTH_SHORT).show();
                });
            }
        }).start();
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 11 && resultCode == Activity.RESULT_OK) {
            ...
            if (file != null) {
                files.add(file);
                adapter.notifyDataSetChanged();
                makeText(getContext(), "Doc created", Toast.LENGTH_SHORT).show();
            }
        }
    }
}