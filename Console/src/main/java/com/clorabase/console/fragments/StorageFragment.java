...
public class StorageFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        com.clorabase.console.databinding.FragmentStorageBinding binding = FragmentStorageBinding.inflate(inflater);
    ...
    public void populateList() {
        new Thread(() -> {
            try {
                var files = Utils.repo.getDirectoryContent(MainActivity.CURRENT_PROJECT + "/Storage");
                for (var file : files) {
                    fileNames.add(file.getName());
                    sizes.add((file.getSize()/(1024*1024)) + " MB");
                }
                new Handler(Looper.getMainLooper()).post(() -> listAdapter.notifyDataSetChanged());
            } catch (IOException e) {
                if (!(e instanceof FileNotFoundException))
                    new Handler(Looper.getMainLooper()).post(() -> Toast.makeText(getContext(), "Failed to read storage. Error :-\n" + e.getMessage(), Toast.LENGTH_LONG).show());
            }
        }).start();
    }
}
