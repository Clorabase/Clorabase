        ...
        binding.delete.setOnClickListener(v -> {
            Utils.read(MainActivity.CURRENT_PROJECT + "/storage.prop", bytes -> {
                ...
                for (var i = 0; i < lines.length; i++) {
                    var line = lines[i];
                    if (line.startsWith(names.get(position))) {
                        lines[i] = "";
                    }
                }
                ...
                Utils.update(builder.toString().getBytes(), MainActivity.CURRENT_PROJECT + "/storage.prop", new Utils.AsyncCallback() {
                    @Override
        ...
        binding.download.setOnClickListener(v -> {
            ...
            ClorabaseStorage.download(MainActivity.CURRENT_PROJECT, names.get(position), context.getExternalFilesDir(null), new ClorabaseStorage.ClorabaseStorageCallback() {
                ...
                public void onComplete() {
                    ...
                    cv.put(MediaStore.Files.FileColumns.TITLE, names.get(position));
