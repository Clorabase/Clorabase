                ...
                if (item.getTitle().equals("Download")) {
                    ...
                    ClorabaseStorage.delete(MainActivity.CURRENT_PROJECT,file.name, new ClorabaseStorageCallback() {
                        ...
                        public void onComplete() {
                            try {
                                StorageFragment.updateStructure(jsonObject -> {
                                    try {
                                        var actualFileName = file.name.substring(file.name.lastIndexOf('_')+1);
                                        StorageFragment.gotoPath(jsonObject,file.path).remove(actualFileName);
                                ...
                                Toast.makeText(context, "IO error occured", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                }
