...
public class DatabaseUtils {
    ...
    protected static synchronized void createFile(Map<String, Object> content, String path) throws Exception {
        ...
        if (data.length > 1024 * 1024)
            throw new IOException("Document size must be less then 1 MB. Current size is " + data.length/1024 + " KB");
    ...
    protected static List<GitFile> getDocuments(String path) throws Exception {
        try {
            var repo = GithubUtils.getRepository();
            ...
            for (GHTreeEntry file : files) {
                ...
                var gitFile = new GitFile(fileName,asMap(new JSONObject(getFileContent(file.getPath()))));
