...
public class ClorastoreDatabase {
    ...
    public static ClorastoreDatabase getInstance(GHRepository repository, String project, String user) {
        if (INSTANCE == null) {
            DatabaseUtils.init(repository, project,user);
    ...
    public Task<Void> document(@NotNull String name, @NotNull Map<String, Object> fields) {
        return Tasks.call(executor, () -> {
            if (name.contains("/") || name.contains(" "))
                throw new IllegalArgumentException("Document name cannot contain '/' or space");

            var finalName = name + ".doc";
            var content = DatabaseUtils.getContent(PATH + finalName);
            if (content.isEmpty())
                DatabaseUtils.createFile(fields, PATH + finalName);
            else
                DatabaseUtils.updateFile(new JSONObject(content), new JSONObject(fields), PATH + finalName);
            return null;
        });
    }
    ...
    public Task<Map<String, Object>> getData(@NotNull String doc) {
        return Tasks.call(executor, () -> DatabaseUtils.getContent(PATH + doc + ".doc"));
    }
    ...
    public Task<Void> delete(String name) {
        return Tasks.call(executor, () -> {
            DatabaseUtils.deleteFile(PATH + name + ".doc");
            return null;
        });
    }
