...
public class CloremDatabase {
     ...
     */
    public static synchronized CloremDatabase getInstance(@NonNull Context context,@NonNull String project) {
        if (INSTANCE == null) {
            if (!GithubUtils.exists(project + "/config.json"))
                throw new IllegalStateException("Project does not exist. Please create a project from the console first");

            var root = Clorem.getInstance(context.getFilesDir(), "clorabase").getDatabase();
            INSTANCE = new CloremDatabase(root,context);
            INSTANCE.db_name = project;
            INSTANCE.BASE_URL = "https://clorabase.herokuapp.com/clorem/" + project;
